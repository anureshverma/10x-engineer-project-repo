...
class TestPrompts:
    ...
    def test_create_prompt_invalid_collection_id_returns_400(self, client: TestClient, sample_prompt_data):
        response = client.post("/prompts", json={**sample_prompt_data, "collection_id": "nonexistent-collection"})
        assert response.status_code == 400
        assert "collection" in response.json().get("detail", "").lower()

    def test_create_prompt_validation_missing_title_returns_422(self, client: TestClient, sample_prompt_data):
    ...
    def test_create_prompt_validation_missing_content_returns_422(self, client: TestClient, sample_prompt_data):
    ...
    def test_list_prompts_filter_by_collection_id(self, client: TestClient, sample_prompt_data, sample_collection_data):
        ...
        client.post("/prompts", json={**sample_prompt_data, "title": "In C1", "collection_id": c1})
        client.post("/prompts", json={**sample_prompt_data, "title": "In C2", "collection_id": c2})
    ...
    def test_list_prompts_search_query(self, client: TestClient, sample_prompt_data):
        client.post("/prompts", json={**sample_prompt_data, "title": "Python tutorial", "description": "Learn Python"})
        client.post("/prompts", json={**sample_prompt_data, "title": "JavaScript guide", "description": "Learn JS"})
        response = client.get("/prompts", params={"search": "Python"})
        assert response.status_code == 200
        assert response.json()["total"] == 1
        assert "Python" in response.json()["prompts"][0]["title"]

    def test_list_prompts_tag_ids_empty_or_no_match(self, client: TestClient, sample_prompt_data):
        client.post("/prompts", json=sample_prompt_data)
    ...
    def test_update_prompt_invalid_collection_id_returns_400(self, client: TestClient, sample_prompt_data):
    ...
    def test_update_prompt_invalid_tag_ids_returns_400(self, client: TestClient, sample_prompt_data):
        create = client.post("/prompts", json=sample_prompt_data)
        prompt_id = create.json()["id"]
        response = client.put(f"/prompts/{prompt_id}", json={
            "title": sample_prompt_data["title"],
    ...
    def test_patch_prompt_success(self, client: TestClient, sample_prompt_data):
        ...
        response = client.patch(f"/prompts/{prompt_id}", json={"title": "Patched Title Only"})
        assert response.status_code == 200
        data = response.json()
        assert data["title"] == "Patched Title Only"
        assert data["content"] == original_content

    def test_patch_prompt_not_found_returns_404(self, client: TestClient, sample_prompt_data):
        response = client.patch("/prompts/nonexistent-id", json={"title": "Foo"})
        assert response.status_code == 404

    def test_patch_prompt_invalid_collection_id_returns_400(self, client: TestClient, sample_prompt_data):
        create = client.post("/prompts", json=sample_prompt_data)
        prompt_id = create.json()["id"]
        response = client.patch(f"/prompts/{prompt_id}", json={"collection_id": "nonexistent-collection"})
        assert response.status_code == 400
        assert "collection" in response.json().get("detail", "").lower()

    def test_patch_prompt_invalid_tag_ids_returns_400(self, client: TestClient, sample_prompt_data):
        create = client.post("/prompts", json=sample_prompt_data)
        prompt_id = create.json()["id"]
        response = client.patch(f"/prompts/{prompt_id}", json={"tag_ids": ["bad-tag-id"]})
        assert response.status_code == 400
        assert "not found" in response.json().get("detail", "").lower()
...
class TestPromptTags:
    ...
    def test_add_prompt_tag_prompt_not_found_returns_404(self, client: TestClient):
        ...
        tag_id = tag_res.json()["id"]
        response = client.post("/prompts/nonexistent-id/tags", json={"tag_ids": [tag_id]})
        assert response.status_code == 404

    def test_add_prompt_tag_invalid_tag_id_returns_400(self, client: TestClient, sample_prompt_data):
        prompt_res = client.post("/prompts", json=sample_prompt_data)
        prompt_id = prompt_res.json()["id"]
        response = client.post(f"/prompts/{prompt_id}/tags", json={"tag_ids": ["bad-tag-id"]})
        assert response.status_code == 400
        assert "not found" in response.json().get("detail", "").lower()
    ...
    def test_remove_prompt_tag_idempotent_when_tag_not_assigned(self, client: TestClient, sample_prompt_data):
        prompt_res = client.post("/prompts", json=sample_prompt_data)
        prompt_id = prompt_res.json()["id"]
